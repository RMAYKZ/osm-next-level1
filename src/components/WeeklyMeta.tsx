...
export default function WeeklyMeta() {
  ...
  function handleWhatsApp() {
    const ltFwd = lt.forwards === "attack" ? "⬆️ Hücum" : lt.forwards === "holdPos" ? "⏸️ Pozisyon Tut" : lt.forwards === "defHelp" ? "🔙 Savunmaya Yardım" : lt.forwards === "stayBack" ? "🛡️ Geride Kal" : lt.forwards;
    const ltMid = lt.midfield === "attack" ? "⬆️ Hücum" : lt.midfield === "holdPos" ? "⏸️ Pozisyon Tut" : lt.midfield === "defHelp" ? "🔙 Savunmaya Yardım" : lt.midfield === "stayBack" ? "🛡️ Geride Kal" : lt.midfield;
    const ltDef = lt.defence === "attack" ? "⬆️ Hücum" : lt.defence === "holdPos" ? "⏸️ Pozisyon Tut" : lt.defence === "defHelp" ? "🔙 Savunmaya Yardım" : lt.defence === "stayBack" ? "🛡️ Geride Kal" : lt.defence;
    const msg = [
      `🏆 *OSM Next Level — Haftalık Meta Taktik*`,
      ``,
      `⚽ *Formasyon:* ${fmBase}${fmStyle ? ` (${fmStyle})` : ""}`,
      ``,
      `🎮 *Slider Değerleri*`,
      `⚡ Baskı: *${P}*`,
      `🎯 Stil: *${S}*`,
      ...
      `Orta saha: ${ltMid}`,
      `Savunma: ${ltDef}`,
      `Ofsayt tuzağı: ${lt.offsides ? "✅ Açık" : "❌ Kapalı"}`,
      `Marking: ${lt.marking === "area" ? "Bölge" : "Adam"}`,
      ``,
      `🔗 Daha fazlası için: https://www.osmantrenor.com`,
    ].join("\n");
      ...
      >
        ...
        <motion.button
          ...
          style={{
            ...
            color: "#25D366", fontWeight: 800, fontSize: 13,
