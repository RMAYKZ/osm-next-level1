...
// ── Stick figure ─────────────────────────────────────────────────────────────
interface StickProps { color: string; phase: number; speed: number }

function StickFigure({ color, phase, speed }: StickProps) {
  const dur = speed * 0.4;
  return (
    <motion.svg
      width="16" height="26"
    ...
    >
      ...
      {/* Left arm — rotate around shoulder pivot (0, -10) */}
      <motion.g
      ...
      {/* Left leg — rotate around hip (0, -5) */}
      <motion.g
...
// ── Player config ─────────────────────────────────────────────────────────────
// Each player: base position + drift range + animation params
const PLAYERS = [
  // GK
...
// ── Ball ──────────────────────────────────────────────────────────────────────
const BALL_PATH = [
  [50, 71], [50, 19], [76, 23], [50, 47], [24, 23],
  [50, 71], [26, 50], [18, 72], [50, 88], [82, 72],
  [74, 50], [50, 47], [50, 19], [50, 71],
] as [number, number][];
...
function Ball() {
  ...
  useEffect(() => {
    ref.current = setInterval(() => setBi(i => (i + 1) % BALL_PATH.length), 1800);
...
// ── Running player ────────────────────────────────────────────────────────────
function RunningPlayer({ x, y, dx, dy, dur, phase, isGK }: typeof PLAYERS[0]) {
  const color = isGK ? "rgba(255,255,255,0.9)" : NEON;

  // Drift in a figure-8 / elliptical path around home position
  ...
  return (
    ...
    >
      <StickFigure color={color} phase={phase} speed={dur} />
    </motion.div>
  );
}
